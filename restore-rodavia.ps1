...
.DESCRIPTION
    ...
    - Base de datos rodavia_dev (PostgreSQL)
...
Write-Host "  Rodavia - RESTAURACION DE BACKUP" -ForegroundColor Cyan
Write-Host "======================================================================" -ForegroundColor Cyan
Write-Host ""
...
$RodaviaBackup = Find-LatestBackup -DatabaseName "rodavia_dev" -SearchPattern $BackupDate
$identityBackup = Find-LatestBackup -DatabaseName "identity_dev" -SearchPattern $BackupDate

if (-not $RodaviaBackup) {
    Write-Log "No se encontro backup de rodavia_dev para fecha: $BackupDate" "ERROR"
    Write-Log "Backups disponibles:"
    Get-ChildItem -Path $dbBackupPath -Filter "rodavia_dev_*" | ForEach-Object { Write-Log "  - $($_.Name)" }
    exit 1
}
...
Write-Log "Backup encontrado (rodavia_dev): $($RodaviaBackup.Name)" "SUCCESS"
...
if (-not $Force) {
    ...
    Write-Host "  - $($RodaviaBackup.Name)" -ForegroundColor Gray
...
# Restaurar rodavia_dev (puerto 5433)
$RodaviaSuccess = Restore-Database -DatabaseName "rodavia_dev" -Port 5433 -BackupFile $RodaviaBackup
...
if ($configBackup) {
    try {
        ...
        $tempConfigDir = Join-Path $env:TEMP "rodavia_restore_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
        ...
        foreach ($file in $restoredFiles) {
            if ($file.Name -match "^appsettings") {
                ...
                if ($file.Name -match "Scraper") {
                    $destination = "Rodavia.Scraper\$($file.Name)"
                } else {
                    $destination = "Rodavia.Web\Rodavia.Web\$($file.Name)"
                }
...
$totalSuccess = $RodaviaSuccess -and $identitySuccess -and $configSuccess

if ($RodaviaSuccess) {
    Write-Host "  [OK] Base de datos rodavia_dev" -ForegroundColor Green
} else {
    Write-Host "  [FAIL] Base de datos rodavia_dev" -ForegroundColor Red
}
...
if ($totalSuccess) {
    ...
    Write-Host "  1. Reiniciar aplicacion: dotnet run --project Rodavia.Web/Rodavia.Web" -ForegroundColor Gray
